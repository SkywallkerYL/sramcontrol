...
//     //数据包个数fifo读
//     val unpackedNumFifoRead = Vec(priornum,Flipped(new ReaderIO(DataWidth)))
//     //数据包长度fifo读
//     val unpackedLenFifoRead = Vec(priornum,Flipped(new ReaderIO(lenwidth)))
//     //数据包地址fifo读
//     val unpackedAddrFifoRead = Vec(priornum,Flipped(new ReaderIO(AddrWidth)))
//     //仲裁模块读数据请求
...
//   //当前处理的数据 
//   val unpackDataNumin = io.unpackedNumFifoRead(prior).data
  
//   val unpackDataLenin = io.unpackedLenFifoRead(prior).data
//   val unpackDataAddrin = io.unpackedAddrFifoRead(prior).data
...
//   //主状态机
//   val sIdle :: sAddr :: sCrc :: sData :: sCheck :: sUpdate :: Nil = Enum(4)
...
//         io.unpackedNumFifoRead(priorMux).read := true.B
//         io.unpackedLenFifoRead(priorMux).read := true.B
//         io.unpackedAddrFifoRead(priorMux).read := true.B
//         unpackDataLen := 0.U
//         unpackDataNum := 0.U
//         //注意fifo的模型要保持数据 
//         //就是读取一次，数据还在，直到下一次读取
